// pages.jsx — all page components for Invitaura
...
// ── Event type colors ────────────────────────────────────────────────────────
const eventColors = ['#7B5EA7','#C5607E','#5B8FA8','#6B8F4E'];
const eventIcons = ['◇','✦','☽','✶'];

// ── HomePage ──────────────────────────────────────────────────────────────────
function HomePage({ lang, setPage, theme, feel = {} }) {
  const t = T[lang];
  const isLight = theme === 'champagne';
  const bg = isLight ? '#F7F3E9' : theme === 'midnight' ? '#06080f' : '#0c0a06';
  const fg = isLight ? '#1a1208' : '#F7F3E9';
  const gold = feel.gold || '#D4AF37';

  const [heroVisible, setHeroVisible] = React.useState(false);
  React.useEffect(() => { setTimeout(() => setHeroVisible(true), 100); }, []);
  ...
  return (
    <div style={{ color: fg, minHeight:'100vh' }}>
      ...
      <section data-screen-label="01 Hero" style={{
        ...
        <div style={{ flex:1, display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center',
          ...
          <div className="soft-reveal" style={{
            ...
            {[
              { icon:'sparkle', n:'+100', label: lang === 'es' ? 'Eventos creados con detalle' : 'Events crafted with detail' },
              { icon:'globe',   n:'+50k', label: lang === 'es' ? 'Invitados alcanzados' : 'Guests reached' },
            ].map((s, i) => (
              <div key={i} className="liquid-glass" style={{
                ...
                <div className="liquid-glass" style={{
                  ...
                  {s.icon === 'sparkle' ? (
                    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#F7F3E9" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round">
                      <path d="M12 3l1.8 4.7L18.5 9.5l-4.7 1.8L12 16l-1.8-4.7L5.5 9.5l4.7-1.8z"/>
                      <path d="M19 14l.7 1.7L21.5 16.5l-1.7.8L19 19l-.8-1.7L16.5 16.5l1.7-.8z"/>
                    </svg>
                  ) : (
                    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#F7F3E9" strokeWidth="1.6">
                ...
                <span className="font-heading" style={{
                  fontStyle:'italic', fontSize:38, color:'#F7F3E9', lineHeight:0.9, letterSpacing:'-0.02em',
                }}>{s.n}</span>
                <span className="font-body" style={{
          ...
          <div style={{ display:'flex', alignItems:'center', gap:48, flexWrap:'wrap', justifyContent:'center' }}>
            {[
              ...
              <span key={i} className="font-heading" style={{
                fontStyle:'italic', fontSize:26, color:'#F7F3E9', letterSpacing:'-0.01em',
              }}>{name}</span>
            ))}
          </div>
        </div>
      ...
      {/* Philosophy */}
      <section data-screen-label="02 Philosophy" style={{ padding:`${Math.round(120*(feel.spacing||1))}px 48px`, maxWidth:900, margin:'0 auto', textAlign:'center', zIndex:1, position:'relative' }}>
        <RevealOnScroll>
      ...
      {/* Event types — liquid-glass cards matching hero stats */}
      <section data-screen-label="03 Events" style={{ padding:`${Math.round(80*(feel.spacing||1))}px 24px ${Math.round(120*(feel.spacing||1))}px`, maxWidth:1180, margin:'0 auto', zIndex:1, position:'relative' }}>
        <RevealOnScroll>
          ...
          <h2 className="font-heading" style={{ fontStyle:'italic', fontSize:'clamp(36px, 5vw, 60px)', lineHeight:0.95, letterSpacing:'-0.03em', color:'#F7F3E9', textAlign:'center', marginBottom:56 }}>
        ...
        <div style={{ display:'flex', alignItems:'stretch', gap:16, flexWrap:'wrap', justifyContent:'center' }}>
          {t.events.map((ev, i) => {
            const icons = [
              // Bodas — two rings
              <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#F7F3E9" strokeWidth="1.6"><circle cx="9" cy="14" r="6"/><circle cx="15" cy="14" r="6"/><path d="M7 5l2 3M17 5l-2 3" strokeLinecap="round"/></svg>,
              // XV Años — sparkle/crown
              <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#F7F3E9" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round"><path d="M3 18l2-9 4 5 3-9 3 9 4-5 2 9z"/><line x1="3" y1="21" x2="21" y2="21"/></svg>,
              // Bautizos — dove
              <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#F7F3E9" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round"><path d="M3 11c4-6 11-7 18-4-3 5-8 7-12 7-3 0-5-1-6-3z"/><path d="M9 14c-1 2-1 4 0 6"/><circle cx="17" cy="8" r="0.6" fill="#F7F3E9"/></svg>,
              // Graduaciones — cap
              <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#F7F3E9" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round"><path d="M2 9l10-4 10 4-10 4z"/><path d="M6 11v5c0 1.5 3 3 6 3s6-1.5 6-3v-5"/><line x1="22" y1="9" x2="22" y2="14"/></svg>,
            ];
            const subs = lang === 'es'
...
// ── PortfolioPage ─────────────────────────────────────────────────────────────
function PortfolioPage({ lang, theme, feel = {} }) {
  const t = T[lang];
  const isLight = theme === 'champagne';
  const bg = isLight ? '#F7F3E9' : theme === 'midnight' ? '#06080f' : '#0c0a06';
  const fg = isLight ? '#1a1208' : '#F7F3E9';
  ...
  return (
    <div style={{ color:fg, minHeight:'100vh', paddingTop:140 }}>
      <div style={{ maxWidth:1280, margin:'0 auto', padding:'0 36px 120px' }}>
        {/* Header */}
        <RevealOnScroll>
          <div style={{ textAlign:'center', marginBottom:64 }}>
            ...
            <h1 style={{ fontFamily:'Cinzel, serif',
              fontSize:'clamp(28px, 4vw, 52px)', fontWeight:400, letterSpacing:6, lineHeight:1.1,
            ...
            return (
              <RevealOnScroll key={i} delay={i * 0.08}>
                <div
                  ...
                  {/* Image slot (right half) */}
                  <div style={{
                    position:'absolute', top:20, right:20, bottom:'42%', width:'42%',
                    ...
                    <image-slot
                      id={`demo-${i}`}
                      placeholder={lang === 'es' ? `Foto ${demo.type.toLowerCase()}` : `${demo.type} photo`}
                      shape="rounded"
                      radius="18"
                      style={{ width:'100%', height:'100%', display:'block',
                        background:`linear-gradient(135deg, ${th.accent}20, ${th.accent2}10)` }}
                    />
                  </div>
                  ...
                  <div style={{ position:'absolute', bottom:32, left:32, right:32 }}>
                    <h3 style={{
                      ...
                      fontSize:'clamp(22px, 2.6vw, 32px)', fontWeight:600, letterSpacing:3, lineHeight:1.15,
      ...
      {activeDemo !== null && (
        <div onClick={() => setActiveDemo(null)} style={{
          ...
          <div onClick={e => e.stopPropagation()} style={{
            ...
            <h2 style={{ fontFamily:'Cinzel, serif', fontSize:28, letterSpacing:4, color:'#F7F3E9', marginBottom:8 }}>
              {t.demos[activeDemo].title}
            </h2>
            <p style={{ fontFamily:'Great Vibes, cursive', fontSize:28, color:demoThemes[activeDemo].accent, marginBottom:28 }}>
            ...
            <button style={{
              ...
              border:'none', color:'#0c0a06', cursor:'pointer',
            }}>{lang === 'es' ? 'SOLICITAR ESTE DISEÑO' : 'REQUEST THIS DESIGN'}</button>
          </div>
        </div>
      )}
    </div>
...
// ── PricingPage ───────────────────────────────────────────────────────────────
function PricingPage({ lang, theme, setPage, feel = {} }) {
  const t = T[lang];
  const isLight = theme === 'champagne';
  const bg = isLight ? '#F7F3E9' : theme === 'midnight' ? '#06080f' : '#0c0a06';
  const fg = isLight ? '#1a1208' : '#F7F3E9';
      ...
      <div style={{ maxWidth:1100, margin:'0 auto', padding:'0 48px 120px' }}>
        <RevealOnScroll>
          <div style={{ textAlign:'center', marginBottom:90 }}>
            ...
            <h1 style={{ fontFamily:'Cinzel, serif', fontSize:'clamp(28px, 4vw, 52px)', fontWeight:400, letterSpacing:6, marginBottom:20 }}>
            ...
            <RevealOnScroll key={i} delay={i * 0.12}>
              ...
              >
                ...
                <div style={{ display:'flex', alignItems:'baseline', gap:6, marginBottom:8 }}>
                  <span style={{ fontFamily:'Cormorant Garamond, serif', fontSize:'clamp(44px, 5vw, 64px)', fontWeight:300, color: pkg.highlight ? gold : fg, lineHeight:1 }}>
                    {pkg.price}
                  </span>
                  <span style={{ fontFamily:'Cinzel, serif', fontSize:11, letterSpacing:2, color: isLight ? '#888' : '#888' }}>{pkg.tag}</span>
                </div>
        ...
        <RevealOnScroll delay={0.3}>
          <p style={{ textAlign:'center', fontFamily:'Cormorant Garamond, serif', fontSize:15, color: isLight ? '#888' : '#666', marginTop:48 }}>
            {lang === 'es' ? '* Precios en MXN · IVA no incluido · Cotización personalizada disponible' : '* Prices in MXN · VAT not included · Custom quotes available'}
          </p>
        </RevealOnScroll>
      </div>
    </div>
...
// ── AboutPage ─────────────────────────────────────────────────────────────────
function AboutPage({ lang, theme, setPage, feel = {} }) {
  const t = T[lang];
  const isLight = theme === 'champagne';
  const bg = isLight ? '#F7F3E9' : theme === 'midnight' ? '#06080f' : '#0c0a06';
  const fg = isLight ? '#1a1208' : '#F7F3E9';
      ...
      <div style={{ maxWidth:1100, margin:'0 auto', padding:'0 48px 120px' }}>
        ...
        <RevealOnScroll>
          <div style={{ marginBottom:80 }}>
            ...
            <h1 style={{ fontFamily:'Cinzel, serif', fontSize:'clamp(28px, 4vw, 52px)', fontWeight:400, letterSpacing:5, marginBottom:0, maxWidth:700 }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:80, alignItems:'start', marginBottom:100 }}>
          ...
          <RevealOnScroll direction="right" delay={0.1}>
            <div style={{ display:'flex', flexDirection:'column', gap:2 }}>
              {stats.map((s, i) => (
                <div key={i} style={{
                  padding:'40px 40px', background: isLight ? '#fff' : 'rgba(255,255,255,0.02)',
        ...
        <RevealOnScroll>
          <div style={{ textAlign:'center', paddingTop:60, borderTop:`1px solid ${gold}20` }}>
            <p style={{ fontFamily:'Great Vibes, cursive', fontSize:42, color:gold, marginBottom:24 }}>
...
// ── ContactPage ───────────────────────────────────────────────────────────────
function ContactPage({ lang, theme, feel = {} }) {
  const t = T[lang];
  const isLight = theme === 'champagne';
  const bg = isLight ? '#F7F3E9' : theme === 'midnight' ? '#06080f' : '#0c0a06';
  const fg = isLight ? '#1a1208' : '#F7F3E9';
  ...
  const handleSend = () => {
    ...
    window.open(`https://wa.me/5213312345678?text=${encodeURIComponent(msg)}`, '_blank');
    setSent(true);
  };
    ...
    <div style={{ color:fg, minHeight:'100vh' }}>
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', minHeight:'100vh' }}>
        ...
        <div style={{
          ...
          padding:80, textAlign:'center', position:'relative', overflow:'hidden',
        }}>
          {[...Array(5)].map((_,i) => (
            <div key={i} style={{
          ...
          <div style={{ position:'relative', zIndex:1 }}>
            ...
            <h2 style={{ fontFamily:'Cinzel, serif', fontSize:'clamp(22px, 3vw, 36px)', letterSpacing:6, color:'#F7F3E9', margin:'32px 0 16px' }}>
            ...
            <div style={{ marginTop:48, display:'flex', flexDirection:'column', gap:16 }}>
              {[['✉','contacto@invitaura.com'],['📍','Jalisco, México'],['@','@invitaura']].map(([ic, val], i) => (
        ...
        {/* Right form */}
        <div style={{ padding:'140px 72px 80px', display:'flex', flexDirection:'column', justifyContent:'center' }}>
          {sent ? (
...
Object.assign(window, { HomePage, PortfolioPage, PricingPage, AboutPage, ContactPage, T });
