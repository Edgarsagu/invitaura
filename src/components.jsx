...
// ── ParticleCanvas ──────────────────────────────────────────────────────────
export function ParticleCanvas({ colors = ['#D4AF37','#C5A059','#F7F3E9'], count = 110, mode = 'ember' }) {
...
// ── CustomCursor ─────────────────────────────────────────────────────────────
export function CustomCursor({ color = '#D4AF37' }) {
...
// ── WhatsAppBtn ───────────────────────────────────────────────────────────────
export function WhatsAppBtn({ lang = 'es', phone }) {
...
export function BlurText({ text, className = '', baseDelay = 0, stagger = 0.1, style = {} }) {
...
// ── NavBar ───────────────────────────────────────────────────────────────────
export function NavBar({ page, setPage, lang, setLang, theme }) {
  ...
  return (
    <>
      <nav style={{
        ...
        padding:'0 clamp(16px,4vw,32px)', display:'flex', alignItems:'center', justifyContent:'space-between', gap:12,
      }}>
        {/* Logo orb */}
        <div onClick={() => setPage('home')} className="liquid-glass" style={{
          width:48, height:48, borderRadius:9999, cursor:'pointer', flexShrink:0,
        ...
        <div className="liquid-glass nav-desktop" style={{
        ...
        <button onClick={() => setLang(lang === 'es' ? 'en' : 'es')} className="liquid-glass font-body nav-lang-desktop" style={{
          width:48, height:48, borderRadius:9999, cursor:'pointer', flexShrink:0,
        ...
        <button className="liquid-glass nav-hamburger" onClick={() => setMobileOpen(true)} style={{
          width:48, height:48, borderRadius:9999, border:'none', cursor:'pointer', flexShrink:0,
          background:'rgba(255,255,255,0.01)', color:'#F7F3E9',
          alignItems:'center', justifyContent:'center',
        }}>
      ...
      {mobileOpen && (
        <div className="mobile-menu-overlay">
          <button onClick={() => setMobileOpen(false)} style={{
            ...
            background:'none', border:'none', color:'#F7F3E9',
            fontSize:32, cursor:'pointer', lineHeight:1, fontWeight:300,
          }}>✕</button>

          {items.map(([id, label]) => (
            <button key={id} onClick={() => { setPage(id); setMobileOpen(false) }} style={{
              background:'transparent', border:'none', cursor:'pointer',
              fontFamily:'Cinzel, serif', fontSize:22, letterSpacing:4,
              color: page === id ? '#D4AF37' : 'rgba(247,243,233,0.9)',
              padding:'14px 32px', transition:'color 0.2s',
            }}>{label.toUpperCase()}</button>
          ))}

          <button onClick={() => { setPage('contact'); setMobileOpen(false) }} style={{
            marginTop:20, padding:'14px 40px', borderRadius:9999,
            background:'#D4AF37', border:'none', cursor:'pointer',
            fontFamily:'Cinzel, serif', fontSize:11, letterSpacing:3, color:'#0c0a06',
          }}>{ctaLabel.toUpperCase()}</button>

          <button onClick={() => setLang(lang === 'es' ? 'en' : 'es')} style={{
            marginTop:14, background:'none', border:'1px solid rgba(212,175,55,0.35)',
            borderRadius:9999, cursor:'pointer', padding:'10px 28px',
...
// ── RevealOnScroll ────────────────────────────────────────────────────────────
export function RevealOnScroll({ children, delay = 0, direction = 'up', distance = 28 }) {
...
// ── GoldOrnament ─────────────────────────────────────────────────────────────
export function GoldOrnament({ size = 80, animate = false }) {
  return (
...
// ── SplitText ─────────────────────────────────────────────────────────────────
export function SplitText({ text, style = {}, charStyle = {}, stagger = 0.04, delay = 0 }) {
  return (
...
// ── BackgroundFX ─────────────────────────────────────────────────────────────
export function BackgroundFX({ mode = 'aurora', theme = 'oscuro' }) {
